...
export async function registerCustomerRoutes(app: FastifyInstance, state: AppState) {
  ...
  app.get('/customers/:id/timeline', async (req) => {
    const id = (req.params as { id: string }).id;
  ...
  app.get('/customers/:id/billing', async (req, reply) => {
    const id = (req.params as { id: string }).id;
    const customer = state.dataset.customers.find((c) => c.id === id);
    if (!customer) return reply.status(404).send({ ok: false, error: { code: 'NOT_FOUND', message: 'Customer not found' } });
    ...
    return { invoices, payments, debts, totalDue, currency: customer.accountId ? (invoices[0]?.currency ?? 'USD') : 'USD' };
  });

  app.get('/customers/:id/cases', async (req) => {
    const id = (req.params as { id: string }).id;
    const cases = state.dataset.cases.filter((c) => c.customerId === id);
    return { items: cases };
  });
